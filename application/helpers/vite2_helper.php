...
{
    $content = file_get_contents(APPPATH . '../assets/vue2/.vite/manifest.json');
...
function asset_url($entry)
{
    ...
    return isset($manifest[$entry])
        ? base_url('assets/vue2/' . $manifest[$entry]['file'])
        : '';
}
...
function imports_urls($entry)
{
    ...
    if (!empty($manifest[$entry]['imports'])) {
        foreach ($manifest[$entry]['imports'] as $imports) {
            $urls[] = base_url('assets/vue2/' . $manifest[$imports]['file']);
        }
    }
...
function css_urls($entry)
{
    ...
    if (!empty($manifest[$entry]['css'])) {
        foreach ($manifest[$entry]['css'] as $file) {
            $urls[] = base_url('assets/vue2/' . $file);
        }
    }
